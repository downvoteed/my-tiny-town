...
RoadSquare::RoadSquare(std::string name, const std::string& modelPath, const std::string& texturePath,
	...
	: Model(name, modelPath, position, size, rotation)
{
	...
	this->modelMatrix_ = glm::rotate(this->modelMatrix_, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
void RoadSquare::draw(bool isPicking)
{
	// Bind the shader
	this->shader_->bind();
