...
Cottage::Cottage(std::string name, const std::string& modelPath, const std::string& texturePath,
	...
	: Model(name, modelPath, position, size, rotation)
{
	...
	this->modelMatrix_ = glm::rotate(this->modelMatrix_, glm::radians(rotation), glm::vec3(1.0f, 0.0f, 0.0f));
...
void Cottage::draw(bool isPicking)
{
	this->shader_->bind();
...
void Cottage::rotate(float angle, const glm::vec3& axis)
{
