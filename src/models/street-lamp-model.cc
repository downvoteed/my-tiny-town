...
StreetLamp::StreetLamp(std::string name, const std::string& modelPath, const std::string& texturePath, 
    ...
    : Model(name, modelPath , position, size, rotation) 
{
	...
	normal_ = std::make_unique<Texture>("assets/textures/diff/street-lamp-normal.png");
	roughness_ = std::make_unique<Texture>("assets/textures/diff/street-lamp-roughness.png");
	metallic_ = std::make_unique<Texture>("assets/textures/diff/street-lamp-metallic.png");

	// Create the shader
	this->shader_ = std::make_unique<Shader>("assets/shaders/vertex.glsl", "assets/shaders/fragment.glsl");
}

void StreetLamp::draw(bool isPicking) 
{
	// Bind the shader
	this->shader_->bind();
